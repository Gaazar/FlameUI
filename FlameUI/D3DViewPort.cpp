...
#endif
void D3DViewPort::SetOutput(ID3D11Resource* res)
{
	if (d2dTex) d2dTex->Release();
	auto hr = res->QueryInterface(&dxgiSurface);
	render.context->CreateSharedBitmap(__uuidof(IDXGISurface), dxgiSurface, &BitmapProperties(
		D2D1::PixelFormat(DXGI_FORMAT_B8G8R8A8_UNORM, D2D1_ALPHA_MODE_PREMULTIPLIED)), &d2dTex);
}
